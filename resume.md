...
- **First Asset Management — PostgreSQL / capacity:** 5 growth models (0→100M+ rows, **~19 GB / ~12 GB indexes**); **~90%** forecast accuracy; as data volume grew, **INSERT latency increased from ~2.7 s to 22–77 s** (up to **8.3×**); SLA breach risk **~11 months**. **Proposed table partitioning and index tuning** based on load-test evidence — **development team implemented** — **INSERT dropped from 20–70 s to ~1 s or less** (validated to **2B** rows), eliminating the projected bottleneck.
...
- **Sber** (Russia's largest bank; very similar to **JPMorgan Chase**) — **SAP ERP / SAP BW** (**LoadRunner** / **Performance Center**, **SAP GUI**, **SAP Web**, browser **SAP Fiori** apps), transactional and analytical workloads; long-running engagement: **change-request** analysis, **LoadRunner** script maintenance, **Scrum**, **Jira** / **Confluence**. Short engagements: **REST** portals integrated with **SAP ERP**.  
- **Rosgosstrakh / Guidewire** (major P&C insurer; very similar to **State Farm**, Guidewire InsuranceSuite) — **JMeter**, **LoadIT**: **updates** to the customer-approved **load-test methodology** (document **versions**, **appendices**, **success criteria**, **release** LT schedule); **two-stage** strategy (**PREPROD** → **dedicated load-test stand**) with **extrapolation** to **production** capacity; **HVQ** and **IFL** workloads, **integrating LT into the Guidewire release cycle**; high-load **end-to-end** scenarios; **multichannel** profiles (portals, **API**, **batch**); **motor insurance** (**CASCO**, **OSAGO**, e-policy flows); **Redis** in the load profile; stubs (**Spring Boot**, **MockServer**); **AppDynamics**, **Splunk**; **Jira**, **Confluence**, **Bamboo** release tracking.  
...
- **Leroy Merlin** (home improvement retail; very similar to **Home Depot**) and **Cooper / SberMarket** (grocery delivery; very similar to **Instacart**): **Gatling** (Java/Scala) with **Redis** (Leroy Merlin); Leroy Merlin — **TMS**, **Carrier Portal**, enlarged DB (simulated **2–3 years** of operations), **PostgreSQL** and **Kubernetes** pod monitoring in **Grafana**; **Cooper / SberMarket** — goal: analytics-to-runs cycle **~1.5 months** ahead of retail peak; **no dedicated perf stand** — night runs on **production** (**23:00–03:00**); **5000+** users (B2B/B2C/**mobile**, emulator-recorded scripts); key scenario **~900 → 2600+ orders/h**; seasonal peak **+189%** to baseline **without outages** (Gatling, **Kafka**, K8s, PostgreSQL, ELK, GitLab CI).  
- **GIIS “Electronic Budget” / Federal Treasury** (government budget platform; very similar to U.S. **Treasury** financial systems): regulated environment with strict **security** and **traceability** requirements; customer-**approved LTM**: **P1** load profile from **territorial treasury (TOFK)** statistics with **extrapolation** across the network; **stepwise** ramp to **maximum** throughput, **reliability** testing, **SLA** / **percentile** criteria; **stubbing** adjacent systems (**GIS GMP** on the first LT iteration); goals include **resilience** and **failure-tolerance** of subsystems; **PUR** modules; **NGINX** (**nginx-extras**) with **GOST** and **Lua**; **Redis**; **Telegraf**, **InfluxDB**, **Grafana**; **JMeter**.  
- **Corporate portal (IBS internal)** (consulting firm's employee portal): load testing of a **1C-Bitrix** portal; profile from **traffic analytics**; **authentication** peak; **comparative** runs **with and without Active Directory**; slow **login** analysis.  
